...
function loadAddresses() {
  loading.value = true
  useGarnetApiFetch('users/userAddress').then((response) => { addresses.value = response.UserAddress || [] }).catch((error) => toast.error(t(error?.message) || t(error) || 'خطا در دریافت آدرس‌ها')).finally(() => { loading.value = false })
}
function loadLocations() {
  useGarnetApiFetch('address/indexWithKind', { kind: 2 }).then((response) => { locationGroups.value = (response.Address || []).map((item) => ({ title: item.title, cities: (item.address_children || []).map((city) => ({ id: city.id, title: city.title })) })) }).catch((error) => toast.error(t(error?.message) || t(error) || 'خطا در دریافت شهرها'))
}
...
function saveAddress() { if (!validateForm()) return; saving.value = true; useGarnetApiFetch('users/saveUserAddress', addressPayload()).then((response) => { if (response?.UserAddress) addresses.value = response.UserAddress; closeModal(); toast.success(editing.value ? 'آدرس ویرایش شد' : 'آدرس ذخیره شد') }).catch((error) => toast.error(t(error?.message) || t(error) || 'خطا در ذخیره آدرس')).finally(() => { saving.value = false }) }
function setDefaultAddress(address) { saving.value = true; useGarnetApiFetch('users/saveUserAddress', { id: address.id, title: address.title, kind: address.kind || 1, address_id: address.address_id, postal_code: address.postal_code, description: address.description, lat: address.lat || '0', lon: address.lon || '0', receiver_is_different: !!address.receiver_is_different, is_default: 1, receiver_full_name: address.receiver_is_different ? address.receiver_full_name : currentUser.value.full_name, receiver_mobile: address.receiver_is_different ? address.receiver_mobile : currentUser.value.mobile }).then((response) => { if (response?.UserAddress) { addresses.value = response.UserAddress.map((item) => ({ ...item, is_default: String(item.id) === String(address.id) })) } else { addresses.value = addresses.value.map((item) => ({ ...item, is_default: String(item.id) === String(address.id) })) } toast.success('آدرس پیش‌فرض تغییر کرد') }).catch((error) => toast.error(t(error?.message) || t(error) || 'خطا در تغییر آدرس پیش‌فرض')).finally(() => { saving.value = false }) }
